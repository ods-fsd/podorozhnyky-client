...
export const StoryForm = () => {
  const formik = useFormik({
    initialValues: { title: '', description: '', category: '', storyImage: null },
    ...
    <form onSubmit={formik.handleSubmit}>
      ...
      <div>
        ...
        {formik.touched.storyImage && formik.errors.storyImage && (
          <div style={{ color: 'red' }}>{formik.errors.storyImage as string}</div>
        )}
      </div>

      <button type="submit" disabled={!formik.isValid}>Зберегти історію</button>
    </form>
  );
};